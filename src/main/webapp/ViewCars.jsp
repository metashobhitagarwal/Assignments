...
<body>
	...
	<table>
		...
		<tr>
			...
			<td><%=result %></td>
			<td><a
				href="UpdateCar.jsp?companyName=<%=car.getCompanyName()%>&
						...
						&accessoryKit=<%=car.isAccessoryKitPresent()%>">
					Edit </a></td>

			<td><a
				href="DetailedCarView.jsp?companyName=<%=car.getCompanyName() %>&
						modalNumber=<%= car.getModelNumber()%>&carName=<%= car.getName()%>&fuelCapacity=
						<%=car.getFuelCapacity()%>&milage=<%=car.getMilege()%>&price=<%=car.getPrice()%>&
						roadTax=<%=car.getRoadTax()%>&AC=<%=car.isAcPresent()%>&
						powerSteering=<%=car.isPowerSteeringPresent()%>&accessoryKit=
						<%=car.isAccessoryKitPresent()%> ">
					Detailed View </a></td>
		</tr>
		<%} %>
	</table>
